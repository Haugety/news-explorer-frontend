...
function Navigation({
  ...
  return (
    <nav className="navigation">
      <NavLink to="/" className={`navigation__link navigation__link_${path} ${location.pathname === '/' ? 'navigation__link_active_main' : ''}`}>Главная</NavLink>
      <NavLink to="/saved-news" className={`navigation__link navigation__link_${path} ${location.pathname === '/saved-news' ? 'navigation__link_active_saved-news' : ''}`}>Сохраненные статьи</NavLink>
      ...
      {flag ? '' : <button type="button" className={`button navigation__button navigation__button_logout navigation__button_${path}`}>
        <span className="button__text">Грета</span>
